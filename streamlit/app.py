...
# Page configuration
st.set_page_config(
    page_title="Game Theory Simulations",
    page_icon="🎮",
    layout="wide",
...
st.markdown("""
    ...
    /* Constrain image sizes - but allow GIFs to animate */
    .stImage img {
...
def render_blotto_game_tab():
        ...
        if run_button:
            with st.spinner("Running simulation... This may take a while."):
                try:
                    ...
                    run_id = f"blotto_{int(time.time())}"
                    st.session_state.runs[run_id] = {
                        "game": "blotto",
                        "result": result,
                        "timestamp": time.time()
    ...
    # Check for plot file
    plot_path = Path("demos/blotto/blotto_training.png")
    if plot_path.exists():
        ...
        if "blotto" in [r["game"] for r in st.session_state.runs.values()]:
            ...
            if "result" in latest_run and "final_value" in latest_run["result"]:
                st.metric("Final Win Rate", f"{latest_run['result']['final_value']:.4f}")
    else:
        st.info("No plot available. Run a simulation to generate one.")
